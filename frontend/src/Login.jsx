  ...
  return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4 font-sans relative overflow-hidden">
      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-2xl overflow-hidden z-10 border border-gray-200">
        <div className="bg-gradient-to-r from-red-800 to-red-600 p-8 text-center relative">
        ...
        <div className="p-8 pt-10">
          ...
          {step === 1 && (
            <form onSubmit={handleCredentialsSubmit} className="space-y-6">
              ...
              <button type="submit" disabled={loading} className="w-full bg-red-700 hover:bg-red-800 text-white font-bold py-3.5 rounded-xl transition-all flex items-center justify-center shadow-lg">{loading ? "Doğrulanıyor..." : <>Giriş Yap <ArrowRight size={20} className="ml-2" /></>}</button>
            </form>
          )}
          {step === 2 && (
            <form onSubmit={handleOtpSubmit} className="space-y-8 animate-fade-in py-4">
              ...
              <div className="flex flex-col gap-3"><button type="submit" disabled={loading} className="w-full bg-red-700 hover:bg-red-800 text-white font-bold py-3.5 rounded-xl shadow-lg">{loading ? "Doğrulanıyor..." : <>Onayla ve Gir <ShieldCheck size={20} className="ml-2" /></>}</button><button type="button" onClick={() => { setStep(1); setFormData(prev => ({...prev, otp: ''})); setError(''); refreshCaptcha(); }} className="w-full text-gray-500 font-medium py-2 hover:text-gray-700 text-sm">Giriş Ekranına Dön</button></div>
            </form>
          )}
        </div>
      </div>
