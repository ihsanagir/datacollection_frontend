...
import { 
  ...
  ChevronRight, Edit, Trash2, Eye, CheckCircle, XCircle, Clock, Shield, Building, Activity, MessageCircle, AlertCircle, ChevronLeft, MapPin, Upload, FileSpreadsheet
} from 'lucide-react';
...
const ROLES = { SUPER_ADMIN: 'SuperAdmin', ADMIN: 'Admin', LOCAL_ADMIN: 'LocalAdmin', USER: 'User' };
const STATUS = { PENDING: 0, LOCAL_APPROVED: 1, PUBLISHED: 2, REJECTED: 3 };
...
export default function App() {
  ...
  return (
    <div className="flex h-screen bg-gray-50 font-sans text-gray-800">
        ...
        <div className={`flex items-center justify-center border-b border-red-800 bg-red-700 text-white overflow-hidden transition-all duration-300 ${isSidebarOpen ? 'h-44' : 'h-20'}`}>
          {isSidebarOpen ? (
            <div className="text-center animate-fade-in flex flex-col items-center">
              <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center mb-3 shadow-lg"><img src={logo} alt="Kurum Logosu" className="w-10 h-10 object-contain" /></div>
              <h1 className="font-bold text-lg tracking-wide leading-none">VERİ MERKEZİ</h1><span className="text-[10px] text-red-100 opacity-80 mt-1">YÖNETİM PANELİ</span><span className="text-[10px] bg-red-900 px-2 py-0.5 rounded opacity-90 block mt-2 shadow-sm border border-red-800">{currentUser.role}</span>
            </div>
          ) : (
            <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-md"><img src={logo} alt="Logo" className="w-6 h-6 object-contain" /></div>
          )}
        </div>
        <nav className="flex-1 py-6 px-2 space-y-2 overflow-y-auto overflow-x-hidden">
          {filteredMenuItems.map(item => (<button key={item.id} onClick={() => setActiveTab(item.id)} title={!isSidebarOpen?item.label:""} className={`flex items-center w-full p-3 rounded-lg transition-all duration-200 mb-1 group ${activeTab === item.id ? 'bg-red-50 text-red-700 border-r-4 border-red-600 shadow-sm' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'} ${!isSidebarOpen ? 'justify-center' : ''}`}><span className={`${activeTab === item.id ? 'text-red-600' : 'text-gray-400 group-hover:text-gray-600'}`}>{item.icon}</span>{isSidebarOpen && <span className="ml-3 font-medium whitespace-nowrap">{item.label}</span>}</button>))}
        </nav>
        <div className="p-4 border-t border-gray-100"><button onClick={handleLogout} className={`flex items-center w-full p-2 rounded-lg transition-colors text-gray-600 hover:text-red-600 hover:bg-red-50 ${!isSidebarOpen ? 'justify-center' : ''}`}><LogOut size={20} />{isSidebarOpen && <span className="ml-3 font-medium">Çıkış</span>}</button></div>
      </aside>
      ...
      {showContentModal && <ContentModal onClose={() => setShowContentModal(false)} onSave={handleSaveContent} onBatchSave={handleBatchSaveContent} />}
...
function ContentModal({ onClose, onSave, onBatchSave }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 animate-fade-in">
      <div className="bg-white rounded-xl p-6 w-full max-w-lg space-y-4">
        ...
        <textarea className="w-full border p-2 rounded text-sm" rows="2" placeholder="Soru" onChange={e=>setForm({...form,question:e.target.value})}/>
        <textarea className="w-full border p-2 rounded text-sm" rows="4" placeholder="Cevap" onChange={e=>setForm({...form,answer:e.target.value})}/>
...
function InstitutionManagerView({ currentUser, institutions, onAddInstitution, onAddDepartment }) {
  ...
  useEffect(() => { if (!isSuper) { setDeptForm(prev => ({ ...prev, instId: currentUser.institutionId })); } }, [currentUser, isSuper]);
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {isSuper && (<div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200"><h3 className="font-bold text-lg mb-4 flex items-center"><Building className="mr-2 text-red-600"/> Yeni Kurum Ekle</h3><div className="flex gap-2"><input type="text" className="flex-1 border p-2 rounded text-sm focus:ring-2 focus:ring-red-500 outline-none" placeholder="Kurum Adı" value={newInstName} onChange={(e) => setNewInstName(e.target.value)} /><button onClick={() => { if(newInstName) { onAddInstitution(newInstName); setNewInstName(""); } }} className="bg-red-700 text-white px-4 py-2 rounded text-sm font-medium hover:bg-red-800">Ekle</button></div></div>)}
        <div className={`bg-white p-6 rounded-xl shadow-sm border border-gray-200 ${!isSuper ? 'md:col-span-2' : ''}`}><h3 className="font-bold text-lg mb-4 flex items-center"><MapPin className="mr-2 text-blue-600"/> Yeni Birim Ekle</h3><div className="flex flex-col gap-3"><select className={`border p-2 rounded text-sm ${!isSuper ? 'bg-gray-100 text-gray-500' : ''}`} value={deptForm.instId} onChange={(e) => setDeptForm({...deptForm, instId: e.target.value})} disabled={!isSuper}><option value="">Kurum Seçiniz</option>{institutions.map(i => <option key={i.id} value={i.id}>{i.name}</option>)}</select><div className="flex gap-2"><input type="text" className="flex-1 border p-2 rounded text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Birim Adı (Örn: İnsan Kaynakları)" value={deptForm.name} onChange={(e) => setDeptForm({...deptForm, name: e.target.value})} /><button onClick={() => { if(deptForm.instId && deptForm.name) { onAddDepartment(deptForm.instId, deptForm.name); setDeptForm(prev => ({...prev, name: ""})); } else { alert("Lütfen kurum ve birim adı giriniz."); } }} className="bg-blue-600 text-white px-4 py-2 rounded text-sm font-medium hover:bg-blue-700">Birim Ekle</button></div></div></div>
      </div>
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden"><div className="px-6 py-4 border-b border-gray-200 bg-gray-50"><h3 className="font-bold text-gray-800">Mevcut Kurum ve Birimler</h3></div><div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{institutions.map(inst => (<div key={inst.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow"><h4 className="font-bold text-red-700 border-b pb-2 mb-2 flex items-center"><Building size={16} className="mr-2"/> {inst.name}</h4><ul className="space-y-1">{inst.departments.length === 0 ? <li className="text-gray-400 text-xs italic">Henüz birim yok.</li> : inst.departments.map(dept => (<li key={dept.id} className="text-sm text-gray-600 flex items-center"><ChevronRight size={14} className="text-gray-400 mr-1"/> {dept.name}</li>))}</ul></div>))}</div></div>
    </div>
  );
}
function DashboardView({ currentUser, stats }) { return (<div className="space-y-6"><div className="bg-gradient-to-r from-red-800 to-red-600 rounded-xl p-6 text-white shadow-lg"><h2 className="text-2xl font-bold mb-2">Hoşgeldiniz, {currentUser.name}</h2><p className="opacity-90">Yetki: {currentUser.role}</p></div><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6"><StatCard title="Yayınlanan Sorular" value={stats.approved} icon={<CheckCircle size={24}/>} color="green" /><StatCard title="Onay Bekleyen" value={stats.pending} icon={<Clock size={24}/>} color="yellow" /><StatCard title="Reddedilen Sorular" value={stats.rejected} icon={<XCircle size={24}/>} color="red" /><StatCard title="Kullanıcılar" value={stats.users} icon={<Users size={24}/>} color="purple" /></div></div>); }
function LogsView({ logs }) { const [currentPage, setCurrentPage] = useState(1); const itemsPerPage = 5; const totalPages = Math.ceil(logs.length / itemsPerPage); const currentData = logs.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage); return (<div className="bg-white p-6 rounded shadow"><h3 className="font-bold mb-4">Loglar</h3><ul className="text-xs font-mono min-h-[150px]">{currentData.length === 0 ? <li className="text-gray-400">Log kaydı yok.</li> : currentData.map(l=><li key={l.id}>{l.date} - {l.userName}: {l.action}</li>)}</ul><PaginationControls currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} /></div>); }
function PaginationControls({ currentPage, totalPages, onPageChange }) { if (totalPages <= 1) return null; return (<div className="flex items-center justify-center gap-4 p-4 border-t border-gray-100 bg-gray-50"><button onClick={() => onPageChange(currentPage - 1)} disabled={currentPage === 1} className="p-2 rounded hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"><ChevronLeft size={20} /></button><span className="text-sm font-medium text-gray-600">Sayfa {currentPage} / {totalPages}</span><button onClick={() => onPageChange(currentPage + 1)} disabled={currentPage === totalPages} className="p-2 rounded hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"><ChevronRight size={20} /></button></div>); }
function QuestionsView({ currentUser, data, onOpenModal, onStatusChange, onDelete, onReject }) { const [currentPage, setCurrentPage] = useState(1); const itemsPerPage = 5; const totalPages = Math.ceil(data.length / itemsPerPage); const currentData = data.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage); const canApprove = (content) => { if (currentUser.role === ROLES.LOCAL_ADMIN && content.status === STATUS.PENDING) return true; if (currentUser.role === ROLES.ADMIN) { if (content.status === STATUS.LOCAL_APPROVED) return true; if (content.status === STATUS.PENDING) return true; if (content.status === STATUS.REJECTED) return true; } if (currentUser.role === ROLES.SUPER_ADMIN && content.status !== STATUS.PUBLISHED) return true; return false; }; const canDelete = (content) => { if (currentUser.role === ROLES.SUPER_ADMIN) return true; if (currentUser.role === ROLES.ADMIN && content.institutionId === currentUser.institutionId) return true; if (currentUser.role === ROLES.LOCAL_ADMIN && content.institutionId === currentUser.institutionId && content.departmentId === currentUser.departmentId) return true; if (currentUser.role === ROLES.USER && content.status === STATUS.PENDING) return true; return false; }; return (<section className="bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col h-full"><div className="px-6 py-5 border-b border-gray-200 flex justify-between items-center"><h3 className="text-lg font-bold text-gray-800">Soru / Cevap Listesi</h3><button onClick={onOpenModal} className="px-4 py-2 bg-red-700 text-white rounded-lg text-sm font-medium hover:bg-red-800 flex items-center"><Plus size={16} className="mr-2" /> Yeni Soru Ekle</button></div><div className="overflow-x-auto p-2 min-h-[300px]"><table className="w-full text-left border-collapse"><thead><tr className="bg-gray-50 text-gray-600 text-xs uppercase"><th className="p-4 border-b">Kurum</th><th className="p-4 border-b">Soru</th><th className="p-4 border-b">Cevap</th><th className="p-4 border-b">Durum</th><th className="p-4 border-b text-right">İşlemler</th></tr></thead><tbody className="text-sm">{currentData.length === 0 ? <tr><td colSpan="5" className="p-4 text-center text-gray-500">Kayıt yok.</td></tr> : currentData.map((row) => (<tr key={row.id} className="hover:bg-gray-50 transition-colors"><td className="p-4 font-medium">{row.institution}</td><td className="p-4 font-bold text-gray-800 max-w-xs truncate">{row.question}</td><td className="p-4 text-gray-600 max-w-xs truncate">{row.answer}{row.status === STATUS.REJECTED && row.rejectionReason && (<div className="text-red-500 text-xs mt-1 flex items-center font-medium"><AlertCircle size={10} className="mr-1"/> Nedeni: {row.rejectionReason}</div>)}</td><td className="p-4"><StatusBadge status={row.status} /></td><td className="p-4 text-right space-x-2">{canApprove(row) && <button onClick={() => { if (currentUser.role === ROLES.LOCAL_ADMIN) onStatusChange(row.id, STATUS.LOCAL_APPROVED); else onStatusChange(row.id, STATUS.PUBLISHED); }} className="text-green-600 hover:bg-green-50 p-1 rounded" title="Onayla"><CheckCircle size={18}/></button>}{([ROLES.LOCAL_ADMIN, ROLES.ADMIN, ROLES.SUPER_ADMIN].includes(currentUser.role) && row.status !== STATUS.REJECTED) && <button onClick={() => onReject(row.id)} className="text-red-500 hover:bg-red-50 p-1 rounded" title="Reddet"><XCircle size={18}/></button>}{canDelete(row) && <button onClick={() => onDelete(row.id)} className="text-gray-400 hover:text-red-600 p-1 rounded" title="Sil"><Trash2 size={18}/></button>}</td></tr>))}</tbody></table></div><PaginationControls currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} /></section>); }
